...
- 状态：Accepted（2026-09-14；2026-09-14 更新：对象存储由 MinIO 改为 SeaweedFS，见 [ADR-0005](./ADR-0005-storage-seaweedfs.md)）
...
| **PostgreSQL（JSONB + 关系表 + SKIP LOCKED 队列）+ 对象存储（现 SeaweedFS，见 ADR-0005）** | **采纳** |
...
- 大对象全部对象存储（SeaweedFS，目录规划：`assets` / `sim-logs` / `datasets` / `reports`，见 ADR-0005）。
